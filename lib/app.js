...
removeUser = function(user) {
  ...
  if (Object.keys(rooms[user.room]).length < 1) {
    return delete rooms[user.room];
  }
};
...
io.on('connection', function(socket) {
  ...
  return socket.on('join room', function(user) {
    ...
    socket.on('leave room', function() {
      ...
      return socket.emit('availiable rooms', rooms);
    });
    ...
    socket.on('renegotiation request', function(data) {
      var recipient;
      console.log("renegotiation request from " + user.name + " to " + data.username);
      recipient = io.sockets.connected[rooms[user.room][data.username]];
      return recipient.emit('renegotiation', {
        desc: data.desc,
        username: user.name
      });
    });
